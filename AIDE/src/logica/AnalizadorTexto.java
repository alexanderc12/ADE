...
public class AnalizadorTexto {
	...
	private void extraerMetadatosCabacera() {
		...
		articulo.setVolumen(Integer.parseInt(documento.getElementsByAttributeValue(ATRIBUTO_NOMBRE, VALOR_VOLUMEN)
				.attr(ATRIBUTO_CONTENIDO)));
		articulo.setNumero(Integer.parseInt(documento.getElementsByAttributeValue(ATRIBUTO_NOMBRE, VALOR_NUMERO)
				.attr(ATRIBUTO_CONTENIDO)));
	...
	private void extraerMetadatosTexto() {
		...
		articulo.getPalabrasClave().add(listaPalabras[0]);
	...
	private void extraerListaContenidos() {
		...
		for (int i = 1; i < contenidos.size() - 1; i++) {
			articulo.agregarCapitulo(contenidos.get(i).text());
		}
	}
	...
	private void extraerContenidoCapitulos() {
		...
		for (int i = 0; i < articulo.getListaCapitulos().size(); i++) {
			inicioCapitulo = texto.indexOf(articulo.getListaCapitulos().get(i))
					+ articulo.getListaCapitulos().get(i).length();
			if (i == articulo.getListaCapitulos().size() - 1) {
				finCapitulo = texto.indexOf(INDICE_FIN_CAPITULOS);
			} else {
				finCapitulo = texto.indexOf(articulo.getListaCapitulos().get(i + 1));
			}
			articulo.agregarContenido(texto.substring(inicioCapitulo, finCapitulo).replaceAll(EXP_REG_NO_HTML, "")
	...
	private void extraerReferencias() {
		...
		for (Element referencia : referencias) {
			...
			if (texto.startsWith(INICIO_REFERENCIA) && texto.endsWith(FIN_REFERENCIA)) {
